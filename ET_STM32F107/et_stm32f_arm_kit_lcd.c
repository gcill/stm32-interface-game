...
#define CS_High					HAL_GPIO_WritePin(LCD_CS_GPIO_PORT, LCD_CS_GPIO_PIN, GPIO_PIN_SET);	
...
extern uint16_t map[100][100];
//extern HAL_SPI_MspInit(SPI_HandleTypeDef* hspi)
...
{ 
  ...
  HAL_GPIO_WritePin(LCD_BL_GPIO_PORT, LCD_BL_GPIO_PIN, GPIO_PIN_SET);   																		//Back Light ON     
...
{ 
  ...
  HAL_GPIO_Init(LCD_SPI_GPIO_PORT, &GPIO_InitStruct);
	...
	//
	HAL_GPIO_WritePin(LCD_SPI_GPIO_PORT, LCD_SPI_MOSI_PIN, GPIO_PIN_RESET);												// MOSI = Low(Command)	  	
...
//   HAL_GPIO_Init(LCD_SPI_GPIO_PORT, &GPIO_InitStruct);
  ...
  //	
	//Start Send Command Byte to SPI 
	HAL_SPI_Transmit(&hspi3, (uint8_t *) &GLCD_Command, 1, LCD_Timeout);
...
{ 
  ...
  HAL_GPIO_Init(LCD_SPI_GPIO_PORT, &GPIO_InitStruct);
	...
	HAL_GPIO_WritePin(LCD_CS_GPIO_PORT, LCD_CS_GPIO_PIN, GPIO_PIN_RESET);													// CS = Low
	HAL_GPIO_WritePin(LCD_SPI_GPIO_PORT, LCD_SPI_SCK_PIN, GPIO_PIN_SET);													// SCK = High(Strobe D/C)
	HAL_GPIO_WritePin(LCD_SPI_GPIO_PORT, LCD_SPI_SCK_PIN, GPIO_PIN_RESET);	
	
//	LCD_SPI_GPIO_PORT->BSRR = LCD_SPI_MOSI_PIN;
//	LCD_SPI_GPIO_PORT->BSRR = LCD_SPI_SCK_PIN<<16;
//	LCD_CS_GPIO_PORT->BSRR = LCD_CS_GPIO_PIN<<16;
//	LCD_SPI_GPIO_PORT->BSRR = LCD_SPI_SCK_PIN;
//	LCD_SPI_GPIO_PORT->BSRR = LCD_SPI_SCK_PIN<<16;
	//
	//Swap SCK3,MOSI3	= SPI Function
	HAL_SPI_MspInit(&hspi3);
...
//	HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
	...
	//
	//Start Send Data Byte to SPI 
 	HAL_SPI_Transmit(&hspi3, (uint8_t *) &GLCD_Data, 1, LCD_Timeout);
	while(HAL_SPI_GetState(&hspi3) == HAL_SPI_STATE_BUSY_TX)
  {
  }
...
void LCD_Draw_test(void){
  ...
  LCD_SetCursor(50,50); 			
	LCD_WriteCommand(0x2C);    																																			//Memmory Write  																											// Memory Write							
  for(index = 0; index < ((long)500); index++)
  {
		Color = rand()%5000;
		LCD_WriteData16(Color);
...
void draw_map(uint16_t map[100][100]){

   for(int i=0;i==100;i++){
	     for(int j=0;j==100;j++){		 
			LCD_SetCursor(i, j);
			LCD_WriteCommand(0x2C);
      LCD_WriteData16(map[i][j]);
			 }
	 }
}
...
void test50(uint8_t offsetX,uint16_t offsetY,uint16_t cc){

     for(int i=25+offsetX;i<75+offsetX;i++){
	     for(int j=25+offsetY;j<75+offsetY;j++){		 
			LCD_SetCursor(j, i);
			LCD_WriteCommand(0x2C);
      LCD_WriteData16(cc);
